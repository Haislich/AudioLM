...
class W2VBertKmeans(nn.Module):
    ...
    def __init__(self, sample_frequency=16000, dataloader=None, device=None):
        super().__init__()
        self.model, self.processor, self.n_clusters, self.n_init, self.kmeans_verbose = load_model()
    ...
    def forward(self, input_audio, input_audio_hz): 
        ...
        with torch.no_grad(): 
            input_features = self.processor(input_audio, sampling_rate=self.sample_frequency, return_tensors="pt")['input_features']
            input_features = input_features.to(self.device)
            embedding = self.model(input_features, output_hidden_states=True, return_dict=True)
