...
from semantic_acoustic_modeling.utils import cut_the_audios, padding_audio
...
class AudioDataset(Dataset):
    ...
    Args:
        ...
        max_length_audio (int, optional): Maximum length of the audio in seconds. Defaults to None.
    ...
    def __init__(self, path_folder, max_length_audio=None, sample_frequency=24000):
        ...
        self.max_len = max_length_audio * sample_frequency if max_length_audio is not None else None
        self.data = self.get_ready_audio()

    def __len__(self):
        return len(self.data)

    def __collate_audio__(self):
        path_audios = []
    ...
    def get_ready_audio(self):
        ...
        """
        path_audios = self.__collate_audio__()
        data = []
        for elem in path_audios:
            ...
            if audio.size(1) > self.max_len: 
                if self.max_len is not None:
                    audio_list = cut_the_audios(audio, self.max_len)
                    data.extend(audio_list)
                else:
                    raise ValueError("The audio is too long and the max_length_audio is None. Please, set a max_length_audio value.")
...
class AudioDataLoader(DataLoader): 
    ...
    def __init__(self, data_path, batch_size=2, shuffle=False, max_length_audio=20, sample_frequency=24000):
        self.dataset = AudioDataset(data_path, max_length_audio=max_length_audio, sample_frequency=sample_frequency)
